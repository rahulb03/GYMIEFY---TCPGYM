            ...
            <div key={i}>
              <div className={`category-box shop-category-box ${filter?.category?.includes(elem?.slug) ? 'active' : ''}`}>
                <a onClick={() => redirectToCollection(elem?.slug)}>
                  <Avatar data={ASSETS_URL + elem?.categoryThumbnail} placeHolder={placeHolderImage} name={elem?.categoryName} height={45} width={187} customeClass={'shop-category-image'} />
                  <div className='category-box-name'>
                    <h6>{elem?.categoryName}</h6>
                  </div>
                </a>
              </div>
            </div>
